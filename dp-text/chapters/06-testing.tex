...
To validate that the platform meets NFR-5 (response time under 2\,seconds) and NFR-7
(support for concurrent users), load tests were executed using Locust~2.43 against a
...
All endpoints satisfy the 2-second response time requirement (NFR-5) even at the
...
\begin{itemize}
  \item \textbf{DEIMS-SDR site linkage.} eLTER identifies every monitoring site through
    a DEIMS-SDR record that carries geographic boundaries, instrumentation descriptions,
    and responsible organizations. The current data model has no field for a DEIMS-SDR
    identifier, so observations cannot be linked back to the eLTER site registry.
  \item \textbf{Vocabulary alignment.} eLTER uses controlled vocabularies (EnvThes,
    eLTER Standard Observations) for observed properties and ecosystem parameters.
    The current platform stores observed-property metadata as free-text fields inherited
    from FROST, with no vocabulary binding or validation against the eLTER terminology
    services.
  \item \textbf{DAR and OAR registration.} For datasets to appear in the eLTER Digital
    Asset Register, the platform must be able to export dataset metadata conforming to
    the eLTER metadata model. The STA endpoint must also be registrable in the Online
    Asset Register so that the eLTER~CI can link dataset records to live data access.
    Neither capability exists.
  \item \textbf{eLTER~LogIn federation.} Authentication uses a local Keycloak instance.
    Integrating with the eLTER~LogIn service (Perun~AAI) would require configuring
    Keycloak as a broker for the eLTER identity provider and mapping eLTER roles
    to the platform's role model.
  ...
  \item \textbf{FAIR compliance.} eLTER mandates FAIR data principles. The platform
    provides accessibility and interoperability through OGC~STA, but lacks persistent
    identifiers (DOIs), structured provenance records, and machine-readable licensing
    metadata required for registration in the DAR.
\end{itemize}
