...
A pytest-based test suite covering 797~test
functions across 43~test files is included with the \texttt{water-dp} backend, organized into five directories:
...
A Locust load testing suite included in the repository enables throughput measurement
...
\texttt{water-dp} API fetches sensor metadata for a project with many Things, it must
...
operational disadvantages throughout the deployment period. The upstream SMS service
has remained non-functional throughout the duration of this thesis, and no public
roadmap for its restoration has been communicated. The assessment and recommended
migration path are discussed in Section~\ref{sec:impl-tsm-assessment}.
