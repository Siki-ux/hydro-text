...
management platform for the Czech University of Life Sciences Prague within the European
...
\begin{itemize}
  \item \textbf{Automated testing.} A backend suite of 804~unit and integration tests
        ...
        (Section~\ref{sec:test-approach-backend}). Of the 24~functional requirements,
        23 are fully met and one (FR-22, Keycloak group management) is partially met
        (Section~\ref{sec:test-requirements}).

  \item \textbf{Load testing.} Locust-based load tests with 10~and 25~concurrent users
        confirmed that all API endpoints respond within the 2-second threshold.
        Lightweight CRUD operations maintain sub-15\,ms median latency;
...
layer (804~tests; Section~\ref{sec:test-approach-backend}), has gaps in the FROST client integration
