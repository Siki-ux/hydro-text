...
This chapter evaluates the platform against the requirements established in Chapter~\ref{chap:requirements}.
Section~\ref{sec:test-approach} describes the testing strategy. Section~\ref{sec:test-requirements} evaluates each
requirement. Section~\ref{sec:test-performance} discusses performance and scalability. Section~\ref{sec:test-deployment}
documents the CZU deployment and discusses the path to eLTER adoption.
...
functions across 43~test files, organised into five directories:
...
the application from running database initialisation at startup.
...
(76~tests covering role inheritance, batch resolution, and group-based authorisation),
...
Table~\ref{tab:req-evaluation-fr} summarises the evaluation of functional requirements.
...
\begin{longtable}{l p{0.30\linewidth} l p{0.38\linewidth}}
  ...
  FR-2  & CSV file upload ingestion
        ...
        & file-ingest worker; 13~\texttt{test\_things.py} tests cover CSV upload
          endpoint. \\[3pt]
  FR-3  & External SFTP synchronisation
        & Fully met
        & sync-extsftp worker; triggered by cron via MQTT. \\[3pt]
  FR-4  & External REST API synchronisation
        & Fully met
  ...
  FR-7  & Bulk sensor import from CSV template
        ...
        & \texttt{BulkImportDialog} component; 6~\texttt{test\_bulk.py} tests
          with validation and authorisation checks. \\[3pt]
  ...
  FR-9  & Reusable MQTT device types and CSV parsers
        & Fully met
        & SMS catalogue endpoints; parser/device-type CRUD tested in
...
Table~\ref{tab:req-evaluation-nfr} summarises the evaluation of non-functional requirements.
...
\begin{longtable}{l p{0.30\linewidth} l p{0.38\linewidth}}
  ...
  \hline
  \endhead
  NFR-1 & Containerised deployment (Docker + Podman)
        & Fully met
...
\begin{longtable}{l l p{0.55\textwidth}}
  ...
  G-12 & Resolved & SMS catalogue endpoints for parser and device-type CRUD;
...
The platform was not subject to formal performance benchmarking as part of this thesis.
This section discusses the architectural properties that affect performance and identifies
known bottlenecks.
...
The primary performance bottleneck is FROST-Server. As a synchronous Java application
running on Tomcat, FROST processes STA queries sequentially within each thread. When the
\texttt{water-dp} API fetches sensor metadata for a project containing many Things, each
FROST query must complete before the next is dispatched. The \texttt{AsyncThingService}
mitigates this by issuing FROST requests concurrently via \texttt{httpx} connection pooling,
but FROST-Server itself remains the throughput ceiling. Replacing FROST with a lighter STA
...
needed for hosting data from independent research sites. The containerised deployment
...
\begin{itemize}
  ...
  \item \textbf{Cross-site federation.} The platform operates as a single-site
    ...
    queries, and data harmonisation across independently operated instances.
...
rather than workarounds labelled as temporary --- is the more defensible long-term
