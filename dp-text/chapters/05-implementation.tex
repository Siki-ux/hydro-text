...
All three platform components are described in turn below. Section~\ref{sec:impl-tsm} covers the
...
Built on FastAPI, the \texttt{water-dp} API acts as the application-logic layer on top of
...
Built on Next.js~15 with React~19, the frontend portal uses the App~Router architecture. The
...
Recharts library. Interactive zoom is supported via two mechanisms: a
...
An Axios instance in \texttt{lib/api.ts} serves as the API client, with a request interceptor that
...
Geospatial rendering uses MapLibre~GL~JS in three components. Sensor locations appear as clustered GeoJSON markers on a CARTO basemap
...
Three languages are supported by the portal: English, Czech, and Slovak. A custom solution based on a Zustand store (\texttt{useLanguageStore}) with
