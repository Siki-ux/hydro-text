...
FastAPI~\cite{fastapi-ref} was chosen for the data platform backend over Flask and Django. Its
native \texttt{async}/\texttt{await} support is needed for non-blocking database
queries (via SQLAlchemy~2.0 async sessions) and concurrent HTTP calls to FROST-Server and
GeoServer. Pydantic request and response schemas generate OpenAPI documentation automatically,
so a separate API specification does not need to be maintained. Flask lacks built-in
async support and would need additional libraries for request validation. Django was
considered too heavy for a service that acts primarily as an API layer and does not rely on Django's
...
Components and server-side rendering let authentication-protected routes be rendered on
the server, so unauthenticated users never receive page content. File-based
routing cuts boilerplate, and built-in internationalization support simplifies
the multilingual requirement (FR-25). A plain React single-page application would need a
separate server for SSR and manual route configuration. Vue.js was considered, but at the time
of selection its ecosystem for complex applications combining SSR, authentication middleware,
and internationalization was less mature.

\subsection{Time-Series Storage: TimescaleDB}
\label{sec:impl-tech-timescaledb}

TimescaleDB~\cite{timescaledb-ref} was preferred over InfluxDB for time-series storage. As a PostgreSQL
extension, it runs on the same database server that already hosts the TSM configuration
database and the \texttt{water\_dp} application schema; no separate database system
to deploy and maintain. It brings
automatic time-based chunking of hypertables, native compression for older chunks, and full
SQL compatibility, so observations can be joined with configuration
tables without a gateway or translation layer. InfluxDB would have meant a separate
deployment, a different query language (InfluxQL or Flux), and a synchronization mechanism
between the two databases.

\subsection{Geospatial Layer: GeoServer}
\label{sec:impl-tech-geoserver}

GeoServer~\cite{geoserver-ref} handles geospatial layer management. It exposes standards-compliant
OGC Web Map Service (WMS) and Web Feature Service (WFS) interfaces over PostGIS data, fitting
the platform's broader reliance on OGC standards for interoperability.
GeoServer is widely deployed in research institutions and supports workspaces, datastores, and
layer publishing through a REST~API, which the data platform backend calls for programmatic
layer management. Geospatial features reside in PostGIS geometry columns (SRID
4326) and are published through GeoServer to the frontend, which renders them with
MapLibre~GL.

\subsection{Authentication: Keycloak}
\label{sec:impl-tech-keycloak}

Keycloak~\cite{keycloak-ref} was carried over from the upstream TSM stack. It offers a battle-tested OIDC
implementation with realm and client management, group-based access control, and integration
with external identity providers. A two-tier RBAC model splits responsibilities:
Keycloak realm roles define platform-wide privileges (e.g.\ administrator), while the
\texttt{project\_members} table in the \texttt{water\_dp} schema stores per-project roles.
Keycloak groups link to projects through the \texttt{authorization\_provider\_group\_id}
column, letting group membership govern project access without duplicating user management.
JWT tokens are verified by fetching the JSON Web Key Set (JWKS) from the Keycloak endpoint
and checking RS256 signatures.

\subsection{Object Storage: MinIO}
\label{sec:impl-tech-minio}

MinIO~\cite{minio-ref} was also kept from the upstream TSM stack. Its S3-compatible object storage
runs without cloud service dependencies, which is important for on-premise deployments at research
institutions. Each Thing gets its own MinIO bucket for raw data files. When a file lands
in a bucket, MinIO fires an S3~event notification to MQTT, which triggers the file-ingest
worker automatically. MinIO also stores user-uploaded parser scripts and
...
\texttt{PROPERTIES} column. For measurement units, the \texttt{DATASTREAMS} view extracts metadata from the
...
(\texttt{UNIT\_OF\_MEASUREMENT}) that FROST requires. Additionally, the \texttt{OBSERVATIONS} view adds a
\texttt{dataSource: local} marker to the output properties to distinguish locally ingested
observations from those originating through SMS. GeoJSON coordinates are extracted by the \texttt{LOCATIONS} view from the Thing's \texttt{properties} JSON, constructing both the text-based
\texttt{LOCATION} column and the PostGIS \texttt{GEOM} geometry column.

No local equivalent exists for the remaining entities: \texttt{SENSORS}, \texttt{FEATURES\_OF\_INTEREST}, and
\texttt{OBSERVED\_PROPERTIES}. Their views return an empty result
...
A class hierarchy rooted in \texttt{AbcParser} forms the parser framework in \texttt{src/timeio/parser/}, defining the abstract interface: \texttt{do\_parse()},
...
Originally, the \texttt{TSM-TEMP} markers were introduced with the expectation that the upstream SMS
...
In \texttt{main.py}, the application is created with a middleware stack applied in order:
...
responses), Starlette\footnote{Starlette ASGI framework: \url{https://www.starlette.io/}} \texttt{CORSMiddleware}, \texttt{TrustedHostMiddleware}, and
\texttt{SlowAPIMiddleware} (rate limiting). OpenAPI documentation is exposed only when the
application runs in debug mode.

Four Alembic\footnote{Alembic database migration tool: \url{https://alembic.sqlalchemy.org/}} migration files manage the \texttt{water\_dp} schema evolution: the initial
...
URL routing follows the FROST multi-tenancy pattern:
...
For the synchronous variant, an \texttt{@lru\_cache(maxsize=128)} factory keyed by
\texttt{(base\_url, project\_name, version, frost\_server)} to avoid creating redundant
client instances. Its asynchronous counterpart maintains a module-level dictionary cache
with \texttt{httpx.AsyncClient} instances configured for connection pooling (100~maximum
connections, 20~keep-alive).

Concurrent FROST operations, such as fetching Things for multiple projects in parallel
during project list rendering, rely on the asynchronous client via \texttt{AsyncThingService}. In Celery tasks and in endpoints where the overhead of an async context switch
is not justified, the synchronous variant is used instead.
