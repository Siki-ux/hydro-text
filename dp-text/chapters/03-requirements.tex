...
consumption. These groups are illustrated in Figure~\ref{fig:uc-diagrams}.

%% PlantUML source files live in figures/*.puml
%% Regenerate PDFs: plantuml -tpdf figures/*.puml

%% --- Use Case Diagrams (combined) ---
\begin{figure}[htbp]
  \centering
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/uc-ingestion.pdf}
    \centerline{\small (a) Data ingestion}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/uc-management.pdf}
    \centerline{\small (b) Platform management}
  \end{minipage}
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/uc-consumption.pdf}
    \centerline{\small (c) Data consumption}
  \end{minipage}
  \caption{Use case diagrams for the three functional areas of the platform.}
  \label{fig:uc-diagrams}
\end{figure}

The identified use cases are summarized in Table~\ref{tab:usecases}.
%% UC-16 (Run computation) removed — feature decommissioned.

\begin{longtable}{l p{0.60\textwidth} l}
...
\begin{description}
  \item[FR-13] The system shall support configuring QA/QC test pipelines per project, using
    functions from the SaQC\footnote{SaQC --- System for automated Quality Control: \url{https://rdm-software.pages.ufz.de/saqc/}} (Statistical Quality Control) library with tunable parameters.
    \emph{(UC-9)}
...
\noindent\textit{Note:} FR-18 (custom dashboard builder) was removed during requirements
refinement; the built-in Grafana dashboards provided by TSM fulfil this role.
...
\begin{description}
  ...
  \item[NFR-6] \textbf{Maintainability.} The codebase shall use database migrations (Alembic
    for the data platform, Flyway for TSM) to manage schema evolution. The API shall generate
    OpenAPI documentation automatically.

  \item[NFR-7] \textbf{eLTER interoperability alignment.} The architecture shall support
    future integration with the eLTER CyberInfrastructure
    (Section~\ref{sec:bg-elter-ci}) without requiring a fundamental redesign. Specifically,
    the platform shall (a)~expose observation data through the OGC SensorThings API so that
    it can be registered in the eLTER Online Asset Register, (b)~use an authentication layer
    that can be federated with the eLTER~LogIn service, (c)~structure its data model so that
    DEIMS-SDR site identifiers and eLTER controlled vocabularies (EnvThes) can be added as
    extensions, and (d)~maintain per-site deployment independence consistent with eLTER's
    site-autonomy principle.

  \item[NFR-8] \textbf{Unified deployment.} The combined TSM and data platform stacks shall
...
\begin{longtable}{l p{0.62\textwidth} l}
  ...
  G-1 & \textbf{No project-oriented data model.} Time.IO organizes Things by
        ...
        \emph{(FR-10--12)} & Data platform \\[6pt]
  ...
  G-2 & \textbf{No user-facing web portal.} The upstream frontends
        ...
        \emph{(FR-16--19)} & Data platform \\[6pt]
  ...
  G-3 & \textbf{No geospatial layer management.} Time.IO stores geographic
        ...
        \emph{(FR-17, FR-24)} & Data platform \\[6pt]
  ...
  G-6 & \textbf{No alert evaluation subsystem.} Time.IO provides no mechanism
        ...
        \emph{(FR-8, UC-8)} & Data platform \\[6pt]
  ...
  G-7 & \textbf{No unified deployment mechanism.} The TSM and application
        ...
        \emph{(NFR-1, NFR-8)} & Deployment layer \\[6pt]
  ...
  G-8 & \textbf{No Podman rootless support.} The upstream Compose files use
        ...
        \emph{(NFR-1)} & Deployment layer \\[6pt]
  ...
  G-9 & \textbf{No multilingual interface.} The upstream frontends are
        ...
        \emph{(FR-25)} & Data platform \\[6pt]

  G-10 & \textbf{Upstream frontend bugs.} Both \texttt{tsm-frontend} (Django)
         ...
         \emph{(FR-6, FR-7)} & Data platform \\[6pt]

  G-11 & \textbf{Hardcoded parsers and device types.} All file parsers, MQTT
         ...
         \emph{(FR-5, FR-9, NFR-4)} & Data platform \\
...
the \texttt{water-dp} application layer addresses gaps~G-1--G-3 and G-6--G-11,
the TSM adaptations address gaps~G-4 and~G-5, and the \texttt{hydro-deploy} deployment
layer addresses gaps~G-7 and~G-8. Each gap is revisited in the requirements evaluation
in Chapter~\ref{chap:testing}.
