...
This chapter derives the requirements for the platform from the CZU deployment context and performs a gap analysis
against the baseline Time.IO system. Section~\ref{sec:req-usecases} identifies the CZU use cases and stakeholder
...
with the platform: \emph{Researcher} (creates projects, configures sensors, analyses data),
...
The use cases are organised into three groups: data ingestion, platform management, and data
...
The identified use cases are summarised in Table~\ref{tab:usecases}.
...
\begin{longtable}{l p{0.58\textwidth} p{0.18\textwidth}}
  ...
  UC-12 & Visualise time-series data for one or more Datastreams with
          interactive charts, decimation controls, and adjustable Y-axis bounds.
          & Researcher \\
  UC-13 & View sensor locations on an interactive map with colour-coded markers
...
\begin{description}
  ...
  \item[FR-9] The system shall manage MQTT device types and CSV parser definitions as reusable
    catalogue entries that can be assigned to multiple Things. \emph{(UC-7)}
\end{description}
...
\begin{description}
  \item[FR-16] The system shall provide interactive time-series charts with support for multiple
    ...
    \emph{(UC-12)}
  \item[FR-17] The system shall display sensor locations on an interactive map with colour-coded
...
\begin{description}
  \item[NFR-1] \textbf{Containerised deployment.} The entire platform shall be deployable as
...
Table~\ref{tab:gap-analysis} summarises the gap between the Time.IO baseline capabilities and the
...
\begin{longtable}{l p{0.58\textwidth} p{0.19\textwidth}}
  ...
  G-1 & \textbf{No project-oriented data model.} Time.IO organises Things by
  ...
  G-2 & \textbf{No user-facing web portal.} The upstream frontends
        ...
        with time-series visualisation, map display, or alerting exists.
  ...
  G-9 & \textbf{No multilingual interface.} The upstream frontends are
        English-only with no internationalisation framework. Czech and Slovak
