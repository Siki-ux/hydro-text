...
presented here. Section~\ref{sec:arch-overview} lays out the
overall system design and the rationale behind the three-component split.
...
MQTT-driven event architecture, Section~\ref{sec:arch-technology} justifies
the major technology selections, and Section~\ref{sec:arch-deployment} covers the
deployment strategy with Docker Compose multi-file merging and rootless Podman support.
...
\textbf{tsm-orchestration} forms the infrastructure layer: a Mosquitto MQTT broker,
...
\textbf{water-dp} sits on top as the application layer: a FastAPI backend, a Next.js frontend portal,
...
\textbf{hydro-deploy} has no runtime presence. It merges
the Docker Compose files from both stacks into a single project, manages shared secrets, and
exposes a Makefile-based operator interface. Both stacks can still be
deployed and operated individually; hydro-deploy simply lets them run as a single unit.

Keeping the adapted upstream infrastructure separate from the new application logic was
...
it. The deployment orchestrator stays separate so that neither runtime stack depends on it.
...
FastAPI~\parencite{fastapi-ref} was chosen for the \texttt{water-dp} backend over Flask and Django. Its
...
Next.js~15~\parencite{nextjs-ref} with the App Router was chosen for the frontend portal. React Server
...
TimescaleDB~\parencite{timescaledb-ref} was preferred over InfluxDB for time-series storage. As a PostgreSQL
...
GeoServer~\parencite{geoserver-ref} handles geospatial layer management. It exposes standards-compliant
...
layer publishing through a REST~API, which the \texttt{water-dp} backend calls for programmatic
...
Keycloak~\parencite{keycloak-ref} was carried over from the upstream TSM stack. It offers a battle-tested OIDC
...
MinIO~\parencite{minio-ref} was also kept from the upstream TSM stack. Its S3-compatible object storage
